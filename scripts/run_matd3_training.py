# Auto-generated from notebooks/rl_full_pipeline.ipynb cells 2-16
import os
...
ANN_FACTOR = 3888   # 16 bars/day * 243 trading days/year
...
class CFG:
    ...
    # ---------- composite reward ----------
    dd_lambda:               float = 0.5    # Calmar: drawdown penalty weight
    vol_lambda:              float = 30.0   # Sharpe: variance penalty weight
    ...
    # ---------- save ----------
    save_dir: str = 'matd3_composite_models'
...
def delta_to_gate(delta_raw):
    """Maps unconstrained delta_raw -> gate in [0.5, 1.5]."""
    ...
    def update(self, replay: 'ReplayBuffer', batch_size: int = 256) -> dict:
        ...
        with torch.no_grad():
            ...
            for i in range(self.n_agents):
                ...
                next_actions.append(torch.clamp(delta_to_gate(d_next + eps), 0.5, 1.5))
